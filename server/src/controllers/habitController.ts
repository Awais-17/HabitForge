...
export const getHabits = async (req: Request, res: Response) => {
  try {
    ...
    const habits = await Habit.findAll({ where: { userId: userId as any } });
    res.json(habits);
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
...
export const updateHabit = async (req: Request, res: Response) => {
  try {
    ...
    const habit = await Habit.findByPk(habitId as string);
...
export const deleteHabit = async (req: Request, res: Response) => {
  try {
    ...
    await HabitLog.destroy({ where: { habitId: habitId as string } });
...
export const completeHabit = async (req: Request, res: Response) => {
  try {
    ...
    // Create Log
    await HabitLog.create({ habitId: parseInt(habitId as string), userId, completedAt: new Date() });
